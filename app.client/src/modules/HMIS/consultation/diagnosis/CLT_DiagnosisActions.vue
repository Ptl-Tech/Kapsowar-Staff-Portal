<template>
    ...
    <div v-if="actionsProps.scope != undefined || actionsProps.scope == 'multipleRecords'" :class="[pageProps.pageType == 'list'? 'flex-col':'flex-col sm:flex-row','flex gap-0.5']">
        <WButton v-if="header.Status != 'Completed'" @click="OnRequestTests()" class="flex items-center !bg-blue-500" title="request"><CheckIcon class="iconSmall" /><span> Request Tests</span></WButton>
    </div>
</template>
<script>
    ...
    export default {
        ...
        methods: {
            ...
            OnRequestTests() {
                if (!confirm('Are you sure you want to request test?')) { return; }
                var url = this.appConfig.baseApiRoute + "ConsultationRadiology/RequestTests";
                this.$root.loader.isLoading = true;
