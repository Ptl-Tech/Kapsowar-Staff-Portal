<template>
    ...
    <div v-if="actionsProps.scope != undefined || actionsProps.scope == 'multipleRecords'" :class="[pageProps.pageType == 'list'? 'flex-col':'flex-col sm:flex-row','flex gap-0.5']">
        <WButton v-if="header.Status != 'Completed'" @click="OnRegisterReferral()" class="flex items-center !bg-blue-500" title="request"><CheckIcon class="iconSmall" /><span> Register Referral</span></WButton>
    </div>
</template>
<script>
    ...
    export default {
        ...
        methods: {
            ...
            OnRegisterReferral() {
                if (!confirm('Are you sure you want to register referral?')) { return; }
                var url = this.appConfig.baseApiRoute + "CLT_Referrals/RegisterReferral";
                this.$root.loader.isLoading = true;
