<template>
    ...
    <div v-if="actionsProps.scope != undefined || actionsProps.scope == 'multipleRecords'" :class="[pageProps.pageType == 'list'? 'flex-col':'flex-col sm:flex-row','flex gap-0.5']">
        <WButton v-if="header.Status != 'Completed'" @click="OnSendToPharmacy()" class="flex items-center !bg-blue-500" title="request"><CheckIcon class="iconSmall" /><span> Send to Pharmacy</span></WButton>
    </div>
</template>
<script>
    ...
    export default {
        ...
        methods: {
            ...
            OnSendToPharmacy() {
                if (!confirm('Are you sure you want to send to pharmacy?')) { return; }
                var url = this.appConfig.baseApiRoute + "CLT_Prescriptions/SendToPharmacy";
                this.$root.loader.isLoading = true;
