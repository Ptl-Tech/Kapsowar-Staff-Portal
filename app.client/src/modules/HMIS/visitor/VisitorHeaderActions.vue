<template>
  <div :class="[pageProps.pageType == 'list'? 'flex-col':'flex-col sm:flex-row','flex gap-0.5']">
    <WButton v-if="pageProps.pageType == 'form' && (formData.recId == '') && $route.params.action != 'view'" class="!bg-blue-500 rounded-sm" @click="OnSubmit('admit')"><CheckIcon class="iconSmall" />Admit Visitor</WButton>
    <WButton v-if="record != null && record.Status == 'Entered' && $route.params.section == 'gate'" @click="OnClearVisitor()" class="flex items-center !bg-green-500" title="clear"><CheckIcon class="iconSmall" /><span> Clear Visitor</span></WButton>
    <WButton v-if="record != null && record.Status != 'Cleared' && $route.params.section == 'reception' && formData.reasonForVisit == '1'" @click="OnConvertToPatient()" class="flex items-center !bg-yellow-500" title="clear"><CheckIcon class="iconSmall" /><span> Convert To Patient</span></WButton>
    <WRouterLink v-if="record != null && record.Status == 'Open' && $route.params.action != 'edit'" :to="pageProps.formRoute+'/edit?recKey'+record[pageProps.keys.recKey]" class="flex items-center !bg-blue-500" title="Edit"><PencilSquareIcon class="iconSmall" /><span> Edit</span></WRouterLink>
    <WButton v-if="$route.params.action != 'create' && record != null && record.Status != 'Cleared'" @click="OnDeleteRecord()" class="flex items-center !bg-red-500" title="delete"><TrashIcon class="iconSmall" /><span> Delete</span></WButton>
    <WRouterLink v-if="record != null && $route.params.action != 'view' && $route.params.action != 'create'" :to="pageProps.formRoute+'/view?recKey'+record[pageProps.keys.recKey]" class="flex items-center !bg-gray-500" title="View"><EyeIcon class="iconSmall" /><span> View</span></WRouterLink>
  </div>
</template>
<script>
    ...
    export default {
        ...
        methods:{
                        ...
                        if (data && data.valErrors) {
                            ...
                            if (action == "admit") {
                                ...
                                if (source.GuiAllowed) {
                                    this.router.push(this.pageProps.listRoute+"?status=Active-Today");
                                } else {
                                    source.next();
                                }
                            }
          ...
          OnClearVisitor() {
            if (!confirm('Are you sure you want to clear this visitor?')) { return; }
            var url = this.appConfig.baseApiRoute + this.pageProps.controller + "/ClearVisitor";
            this.$root.loader.isLoading = true;
            const requestOptions = {
              method: "POST",
              headers: { 'Content-Type': "application/json" },
            ...
            fetch(url, requestOptions)
              ...
              .then(data => {
                if (data && data.valErrors) {
                  ...
                  this.$root.FnNotification("Cleared successfully", 'bg-green-500', false);
                  this.router.push({ path: this.pageProps.listRoute, query: { status: 'Active' }, replace: true })
                }
                this.$root.loader.isLoading = false;
              }).catch((error) => {
          ...
          OnConvertToPatient() {
            if (!confirm('Are you sure you want to create patient?')) { return; }
            var url = this.appConfig.baseApiRoute +"Reception/ConvertVisitorToPatient";
            this.$root.loader.isLoading = true;
            const requestOptions = {
              method: "POST",
              headers: { 'Content-Type': "application/json" },
            ...
            fetch(url, requestOptions)
              ...
              .then(data => {
                if (data && data.valErrors) {
                  ...
                  this.router.push("/hmis/reception/patient/form/edit?recId=" + data.response);
                }
                this.$root.loader.isLoading = false;
              }).catch((error) => {
                this.$root.errorModal.isShow = true;
