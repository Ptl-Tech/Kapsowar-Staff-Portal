<template>
  <div :class="[pageProps.pageType == 'list'? 'flex-col':'flex-col sm:flex-row','flex gap-0.5']">
    <WButton v-if="pageProps.pageType == 'form' && $route.params.action != 'view'" class="!bg-green-500 rounded-sm" @click="OnSubmit('save')"><CheckIcon class="iconSmall" />Save</WButton>
    <WRouterLink v-if="record != null && record.Activated == false" :to="'/hmis/reception/visit/form/create?patientNo='+record[pageProps.keys.recKey]" class="flex items-center !bg-blue-500" title="create visit"><PencilSquareIcon class="iconSmall" /><span> Create Visit</span></WRouterLink>
...
<script>
    ...
    export default {
        ...
        methods:{
            ...
            OnGetReport() {
                var route = this.appConfig.baseApiRoute + this.pageProps.controller +"/Report?recId="+this.record.No;
          ...
          OnCreateVisit() {
            ...
            var url = this.appConfig.baseApiRoute + "VisitsHeader/CreateVisit";
            this.$root.loader.isLoading = true;
            const requestOptions = {
              method: "POST",
              headers: { 'Content-Type': "application/json" },
