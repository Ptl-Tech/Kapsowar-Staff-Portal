<template>
    <div :class="[pageProps.pageType == 'list'? 'flex-col':'flex-col sm:flex-row','flex gap-0.5']">
        <WButton v-if="record != null && record.Completed != true" class="!bg-green-500 rounded-sm" @click="OnDispatchToDoctor()"><CheckIcon class="iconSmall" />Dispatch To Doctor</WButton>
...
<script>
    ...
    export default {
        ...
        methods:{
          ...
          OnDispatchToDoctor() {
            if (!confirm('Are you sure you want to dispatch?')) { return; }
            var url = this.appConfig.baseApiRoute + this.pageProps.controller + "/DispatchToDoctor";
            this.$root.loader.isLoading = true;
            const requestOptions = {
              method: "POST",
              headers: { 'Content-Type': "application/json" },
            ...
            fetch(url, requestOptions)
              ...
              .then(data => {
                if (data && data.valErrors) {
                  ...
                  this.router.push({ path: this.pageProps.listRoute, query: { status: 'Pending-Today' }, replace: true })
                }
                this.$root.loader.isLoading = false;
              }).catch((error) => {
                this.$root.errorModal.isShow = true;
