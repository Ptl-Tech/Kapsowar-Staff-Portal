<template>
    <div :class="[pageProps.pageType == 'list'? 'flex-col':'flex-col sm:flex-row','flex gap-0.5']">
        <WButton v-if="pageProps.pageType == 'form' && formData.myAction != 'view' && !(header.Status != undefined && header.Closed == true)" class="!bg-blue-500 rounded-sm" @click="OnSubmit('save')"><CheckIcon class="iconSmall" />Save</WButton>
        <WRouterLink :to="pageProps.formRoute+'/edit?parentId='+$route.query.parentKey+'&recId='+record[pageProps.keys.recKey]" v-if="record[pageProps.keys.recKey] != undefined && header.Status == 'Open'" class="flex items-center !bg-blue-500" title="Edit"><PencilSquareIcon class="iconSmall" /><span> Edit</span></WRouterLink>
        <WRouterLink :to="pageProps.formRoute+'/view?parentId='+$route.query.parentKey+'&recId='+record[pageProps.keys.recKey]" v-if="record.Line_No != undefined && $route.params.action != 'view' && pageProps.pageType == 'list'" class="flex items-center !bg-gray-500" title="View"><EyeIcon class="iconSmall" /><span> View</span></WRouterLink>
        <WButton v-if="record.Line_No != undefined && header.Status == 'Open'" @click="OnDeleteRecord()" class="flex items-center !bg-red-500" title="delete"><TrashIcon class="iconSmall" /><span> Delete</span></WButton>
    </div>
</template>
<script>
