<template>
    <div :class="[pageProps.pageType == 'list'? 'flex-col':'flex-col sm:flex-row','flex gap-0.5']">
        <WButton v-if="record.Status != undefined && record.Status != 'Completed'" class="!bg-green-500 rounded-sm" @click="OnDispatchToDoctor()"><CheckIcon class="iconSmall" />Dispatch To Doctor</WButton>
    </div>
</template>
<script>
    ...
    export default {
        ...
        methods: {
            ...
            OnDispatchToDoctor() {
                if (!confirm('Are you sure you want to dispatch to doctor?')) { return; }
