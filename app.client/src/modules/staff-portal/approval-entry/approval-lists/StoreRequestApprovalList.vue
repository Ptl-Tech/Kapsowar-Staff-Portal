<template>
    <ListPageTemplate ref="tpList" :pageProps="this.pageProps" :actionsProps="this.actionsProps" @OnFetchData="OnFetchData($event)">
        ...
        <template v-for="(record,index) in records" #[`tbody-${index}`]>
            ...
            <WTd>{{record.DocDetails != null && record.DocDetails != ""? JSON.parse(record.DocDetails).Request_By_Name:""}}</WTd>
            <WTd>
    ...
    export default {
        ...
        methods: {
            ...
            FnGetRecordLink(recApprovalEntry) {
                var link = "";
                //store
                if (recApprovalEntry.Table_ID == 39005759) {
                    link = "/ess/approval-entry/"+this.status+"/store-request/view/" + recApprovalEntry.Document_No + "/" + recApprovalEntry.Entry_No + "/store-request";
                }
                return link;
            }
        },
