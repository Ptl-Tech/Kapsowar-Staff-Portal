<template>
    <ListPageTemplate ref="tpList" :pageProps="this.pageProps" :actionsProps="this.actionsProps" @OnFetchData="OnFetchData($event)">
        ...
        <template v-for="(record,index) in records" #[`tbody-${index}`]>
            ...
            <WTd>{{record.DocDetails != null && record.DocDetails != ""? JSON.parse(record.DocDetails).Shortcut_Dimension_1_Code:""}}</WTd>
            <WTd>{{record.DocDetails != null && record.DocDetails != ""? JSON.parse(record.DocDetails).Shortcut_Dimension_2_Code:""}}</WTd>
            <WTd>{{record.DocDetails != null && record.DocDetails != ""? JSON.parse(record.DocDetails).Requester_s_Name:""}}</WTd>
            <WTd>{{record.DocDetails != null && record.DocDetails != ""? parseFloat(JSON.parse(record.DocDetails).Amount_Including_VAT).toLocaleString():""}}</WTd>
            <WTd>
    ...
    export default {
        ...
        data(){
            return {
                records:[],
                pageProps: {
                    title: 'Purchase Requests Approvals List',
                    pageType:"list",
                    pKey: 'No',
        ...
        methods: {
            ...
            FnGetRecordLink(recApprovalEntry) {
                var link = "";
                //purchase
                if (recApprovalEntry.Table_ID == 38) {
                    link = "/ess/approval-entry/"+this.status+"/purchase-request/view/" + recApprovalEntry.Document_No + "/" + recApprovalEntry.Entry_No + "/purchase-request";
                }
                return link;
            }
        },
