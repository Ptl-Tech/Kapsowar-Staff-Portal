<template>
    <ListPageTemplate ref="tpList" :pageProps="this.pageProps" :actionsProps="this.actionsProps" @OnFetchData="OnFetchData($event)">
        ...
        <template v-for="(record,index) in records" #[`tbody-${index}`]>
            ...
            <WTd>{{record.DocDetails != null && record.DocDetails != ""? JSON.parse(record.DocDetails).Project_Code:""}}</WTd>
            <WTd>{{record.DocDetails != null && record.DocDetails != ""? JSON.parse(record.DocDetails).Year:""}}</WTd>
            <WTd>{{record.DocDetails != null && record.DocDetails != ""? JSON.parse(record.DocDetails).Quarter:""}}</WTd>
            <WTd>
    ...
    export default {
        ...
        data(){
            return {
                records:[],
                pageProps: {
                    title: 'M&E Projects Report Approvals List',
                    pageType:"list",
                    pKey: 'No',
        ...
        methods: {
            ...
            FnGetRecordLink(recApprovalEntry) {
                var link = "";
                //MEProjects
                if (recApprovalEntry.Table_ID == 52018800 && recApprovalEntry.Details.includes("ME Projects")) {
                    link = "/ess/approval-entry/"+this.status+"/ME-projects-report/view/" + recApprovalEntry.Document_No + "/" + recApprovalEntry.Entry_No + "/ME-projects-report/step-1";
                }
                return link;
            }
        },
