<template>
    <div v-if="record.Employee_No == undefined || record.Employee_No == $root.authUser.userNo" :class="[pageProps.pageType == 'list'? 'flex-col':'flex-col sm:flex-row','flex gap-0.5']">
        ...
        <WButton v-if="record[pageProps.keys.recKey] != undefined && record.Status == 'Pending Approval'" @click="OnDelegateOrCancelApproval('StoreRequest','cancelApproval',record[pageProps.keys.recKey])" class="flex items-center !bg-red-500" title="cancel"><ArrowUturnLeftIcon class="iconSmall" /><span> Cancel Approval</span></WButton>
        <WButton v-if="record[pageProps.keys.recKey] != undefined && record.Status == 'Pending Approval'" @click="OnDelegateOrCancelApproval('StoreRequest','delegateApproval',record[pageProps.keys.recKey])" class="flex items-center !bg-yellow-500" title="delegate"><DocumentIcon class="iconSmall" /><span> Delegate Approval</span></WButton>
