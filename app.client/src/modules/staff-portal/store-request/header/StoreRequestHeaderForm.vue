    ...
    <div>
        <FormPageTemplate :title="pageProps.caption+' - '+$route.params.action">
            <grid>
                ...
                <grid-col>
                    <field-group label="Required Date" :showMandatory="true" :valErrors="valErrors.requiredDate">
                        <WInput type="date" required="true" v-model="form.requiredDate" :formMode="formMode" />
                    </field-group>
                </grid-col>
                <grid-col>
                    <field-group label="Request Description" :showMandatory="true" :valErrors="valErrors.description">
            ...
            <div v-if="record[pageProps.keys.recKey] != undefined">
                <MenuTabs class="py-2">
                    <MenuButton name="lines" :activeTab="activeSubpart" @activeTab="activeSubpart = $event">Store Request Lines</MenuButton>
                </MenuTabs>
...
<script>
    ...
    import Lines from '@/modules/staff-portal/store-request/line/StoreRequestLineList.vue';
                    ...
                    .then(data => {
                        if (data && data.errors) {
                            ...
                            if (this.form.myAction != "create") {
                                ...
                                this.form.requiredDate = this.$root.xFnNavDateObjToISODate(formData.Required_Date);
                                this.form.description = formData.Request_Description;
