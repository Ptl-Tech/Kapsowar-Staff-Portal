<template>
    <div :class="[pageProps.pageType == 'list'? 'flex-col':'flex-col sm:flex-row','flex gap-0.5']">
        <WButton v-if="pageProps.pageType == 'form' && (formData.docNo == 'new' || record.Status == 'Open') && $route.params.action != 'view'" class="!bg-blue-500 rounded-sm" @click="OnSubmit('save')"><CheckIcon class="iconSmall" />Save</WButton>
        <WRouterLink v-if="record != null && record.Status == 'Open' && $route.params.action != 'edit'" :to="pageProps.formRoute+'/edit/'+record.No" class="flex items-center !bg-blue-500" title="Edit"><PencilSquareIcon class="iconSmall" /><span> Edit</span></WRouterLink>
        <WButton v-if="record != null && record.Status == 'Open'" @click="OnSubmit('submit')" class="flex items-center !bg-green-500" title="submit"><PaperAirplaneIcon class="iconSmall" /><span> Submit</span></WButton>
        <WButton v-if="record != null && record.Status == 'Open'" @click="OnDeleteRecord()" class="flex items-center !bg-red-500" title="delete"><TrashIcon class="iconSmall" /><span> Delete</span></WButton>
        ...
        <WButton v-if="record != null && record.Status == 'Pending Approval' && (record.Request_By_No == $root.authUser.EmployeeNo)" class="!bg-red-500 rounded-sm" @click="OnSubmit('cancel')" title="cancel approval"><ArrowUturnLeftIcon class="iconSmall" /><span> Cancel Approval</span></WButton>
        <WButton v-if="record != null && record.Status == 'Pending Approval' && (record.Request_By_No == $root.authUser.EmployeeNo)" class="!bg-teal-500 rounded-sm" @click="OnDelegate()" title="Delegate approval"><ArrowUturnLeftIcon class="iconSmall" /><span> Delegate Approval</span></WButton>
        <WButton v-if="record != null && record.Status == 'Released'" class="!bg-green-500 rounded-sm" @click="OnGetReport()" title="Print/Download"><PrinterIcon class="iconSmall" /><span> Print Request</span></WButton>
    </div>
</template>
<script>
