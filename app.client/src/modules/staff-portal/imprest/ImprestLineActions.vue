<template>
    <div :class="[pageProps.pageType == 'list'? 'flex-col':'flex-col sm:flex-row','flex gap-0.5']">
        ...
        <WRouterLink :to="'/ess/staff-advance/line/form/edit/'+$route.params.no+'/'+record.No" v-if="record.No != undefined && header.Approval_Status == 'Open' && pageProps.pageType == 'list'" class="flex items-center !bg-blue-500" title="Edit"><PencilSquareIcon class="iconSmall" /><span> Edit</span></WRouterLink>
        <WButton v-if="record.No != undefined && header.Approval_Status == 'Open'" @click="OnDeleteRecord()" class="flex items-center !bg-red-500" title="delete"><TrashIcon class="iconSmall" /><span> Delete</span></WButton>
        <WRouterLink :to="'/ess/staff-advance/line/form/view/'+$route.params.no+'/'+record.No" v-if="record.No != undefined && formData.myAction != 'view' && pageProps.pageType == 'list'" class="flex items-center !bg-gray-500" title="View"><EyeIcon class="iconSmall" /><span> View</span></WRouterLink>
    </div>
</template>
<script>
