<template>
    <h4 class="font-semibold py-2">Staff Advance Lines</h4>
    <div v-if="isNew && header.Approval_Status != undefined && header.Approval_Status == 'Open'">
        <WRouterLink :to="'/ess/staff-advance/line/form/create/'+docNo+'/0'" class="flex items-center !bg-blue-500" title="Edit"><PlusIcon class="iconSmall" />Add Staff Advance Line</WRouterLink>
    </div>
        ...
        <template v-for="(record,index) in records" #[`tbody-${index}`]>
            ...
            <WTd>
                <EllipsisMenu :docNo="record.No">
                    <Actions v-if="isActions" :record="record" :header="header" :pageProps="pageProps" @onAction="onAction($event)" @closeModal="onCloseModal()" />
                </EllipsisMenu>
            </WTd>
        </template>
        <template #totalsRow>
...
<script>
    ...
    export default {
        props: { docNo: {default:""},isNew: {default:true},isActions: {default:true} },
        ...
        data(){
            return {
                ...
                pageProps: {
                    ...
                    formRoute: "/ess/staff-advance/line/form",
                    docNo:this.docNo
                },
                lineAction:"",
                lineNo:0,
                actionsProps: { isNew: false,isNewCaption:"Add Staff Advance Line", isEdit: false, isDelete: false,isSearch:false, isFilter: false, isExport: false,isOrderBy:false},
                isLineModal: false,
                header: {},
                linesTotal: 0,
                dimNos:[]
