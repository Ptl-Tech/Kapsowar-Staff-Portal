<template>
    <ListPartPageTemplate ref="tpList" :pageProps="this.pageProps" :actionsProps="this.actionsProps" @OnFetchData="OnFetchData($event)">
        <template #thead>
            <WTh>Imprest Type</WTh>
            <WTh>Account No.</WTh>
            <WTh>Account Name</WTh>
            <WTh>Amount</WTh>
            <WTh>Imprest Holder</WTh>
            <WTh>Actions</WTh>
        </template>
        <template v-for="(record,index) in records" #[`tbody-${index}`]>
            <WTd :linkTo="pageProps.formRoute+'/edit?parentId='+record[pageProps.keys.parentKey]+'&recId='+record[pageProps.keys.recKey]">{{record.Advance_Type}}</WTd>
            <WTd>{{record.Account_No}}</WTd>
            <WTd>{{record.Account_Name}}</WTd>
            <WTd>{{record.Amount}}</WTd>
            <WTd>{{record.Imprest_Holder}}</WTd>
            <WTd>
