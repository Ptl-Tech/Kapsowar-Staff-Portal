<template>
    <div :class="[pageProps.pageType == 'list'? 'flex-col':'flex-col sm:flex-row','flex gap-0.5']">
        <WButton v-if="pageProps.pageType == 'form' && (header.Approval_Status == 'Released' && header.Status == 'Assigned') && formData.myAction != 'view'" class="!bg-blue-500 rounded-sm" @click="OnSubmit('save')"><CheckIcon class="iconSmall" />Save</WButton>
        <WRouterLink :to="pageProps.formRoute+'/edit/'+$route.params.no+'/'+record.No" v-if="record.No != undefined && header.Approval_Status == 'Released' && header.Status == 'Assigned'" class="flex items-center !bg-blue-500" title="Edit"><PencilSquareIcon class="iconSmall" /><span> Edit</span></WRouterLink>
        ...
        <WButton v-if="record != null && header.Status == 'Assigned'" @click="OnDeleteRecord()" class="flex items-center !bg-red-500" title="delete"><TrashIcon class="iconSmall" /><span> Delete</span></WButton>
    </div>
</template>
<script>
