<template>
    <div>
        <FormPageTemplate :title="$route.params.action+' Staff Claim'">
            ...
            <grid>
                ...
                <grid-col>
                    <field-group label="Actual Mission Start Date" showMandatory="true" :valErrors="valErrors.actualStartDate">
                        <WInput type="date" required="true" v-model="form.actualStartDate" :formMode="formMode" :max="new Date().toISOString().split('T')[0]" />
                    </field-group>
                </grid-col>
                <grid-col>
                    <field-group label="Actual Date of return from Mission" showMandatory="true" :valErrors="valErrors.actualReturnDate">
                        <WInput type="date" required="true" v-model="form.actualReturnDate" :formMode="formMode" :max="new Date().toISOString().split('T')[0]" />
                    </field-group>
                </grid-col>
            ...
            <hr class="py-2" />
            <ClaimLinesList v-if="form.myAction != 'create' && form.docNo != 'new'" :docNo="form.docNo" />
            ...
            <ClaimAttachmentsList v-if="form.myAction != 'create' && form.docNo != 'new'" :docNo="form.docNo" />
...
<script>
    ...
    import Actions from '@/modules/ess/staff-claim/ClaimActions.vue';
    import ClaimLinesList from '@/modules/ess/staff-claim/ClaimLineList.vue';
    import ClaimAttachmentsList from '@/modules/ess/staff-claim/ClaimAttachmentList.vue';
    ...
    export default {
        components: { Actions, ...W, ClaimLinesList, ClaimAttachmentsList, ...Apr, TomSelectDims, MpesaSheetLines },
        setup() {
            const { router, loadTomSelects, setDropdownDims, updateDimensionSet,cpl_FnAutoSaveFormData, cpl_FnSetInitialFormData } = useFormComposable();
            return { router, loadTomSelects, setDropdownDims, updateDimensionSet,cpl_FnAutoSaveFormData, cpl_FnSetInitialFormData };
        },
        data() {
            return {
                pageProps: {
                    title: 'Staff Claim',
                    pageType: "form",
                    controller: 'StaffClaim',
                    formRoute: "/ess/staff-claim/form",
                    listRoute: "/ess/staff-claim/list",
                },
                form: {
                    docNo: '',
                    myAction: '',
