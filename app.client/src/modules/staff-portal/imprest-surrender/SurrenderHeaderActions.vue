<template>
    <div :class="[pageProps.pageType == 'list'? 'flex-col':'flex-col sm:flex-row','flex gap-0.5']">
        ...
        <WButton v-if="record != null && record.Approval_Status == 'Pending Approval' && (record.Requester_Employee_No == $root.authUser.EmployeeNo)" class="!bg-red-500 rounded-sm" @click="OnSubmit('cancel')" title="cancel approval"><ArrowUturnLeftIcon class="iconSmall" /><span> Cancel Approval</span></WButton>
        <WButton v-if="record != null && record.Approval_Status == 'Pending Approval' && (record.Requester_Employee_No == $root.authUser.EmployeeNo)" class="!bg-teal-500 rounded-sm" @click="OnDelegate()" title="Delegate approval"><ArrowUturnLeftIcon class="iconSmall" /><span> Delegate Approval</span></WButton>
        <WButton v-if="record != null && record.Approval_Status == 'Released'" class="!bg-green-500 rounded-sm" @click="OnGetReport()" title="Print/Download"><PrinterIcon class="iconSmall" /><span> Print Advance</span></WButton>
    </div>
</template>
<script>
