<template>
    <div v-if="record.Employee_No == undefined || record.Employee_No == $root.authUser.userNo" :class="[pageProps.pageType == 'list'? 'flex-col':'flex-col sm:flex-row','flex gap-0.5']">
        <WButton v-if="pageProps.pageType == 'form' && formData.myAction != 'view' && (record.Status == undefined || record.Status == 'Pending')" class="!bg-blue-500 rounded-sm" @click="OnSubmit('save')"><CheckIcon class="iconSmall" />Save</WButton>
        <WButton v-if="record.Status != undefined && pageProps.pageType == 'form' && formData.myAction != 'view' && (record.Status == undefined || record.Status == 'Pending')" class="!bg-green-500 rounded-sm" @click="OnSubmit('submit')"><CheckIcon class="iconSmall" />Save & Send for Approval</WButton>
        ...
        <WButton v-if="record[pageProps.keys.recKey] != undefined && record.Status == 'Pending Approval'" @click="OnDelegateOrCancelApproval('ImprestSurrender','cancelApproval',record[pageProps.keys.recKey])" class="flex items-center !bg-red-500" title="cancel"><ArrowUturnLeftIcon class="iconSmall" /><span> Cancel Approval</span></WButton>
        <WButton v-if="record[pageProps.keys.recKey] != undefined && record.Status == 'Pending Approval'" @click="OnDelegateOrCancelApproval('ImprestSurrender','delegateApproval',record[pageProps.keys.recKey])" class="flex items-center !bg-yellow-500" title="delegate"><DocumentIcon class="iconSmall" /><span> Delegate Approval</span></WButton>
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            OnDownloadReport() {
                var url = this.appConfig.baseApiRoute + "Reports/ImprestSurrenderReport";
                this.$root.loader.isLoading = true;
                const requestOptions = {
                    method: "POST",
                    headers: { 'Content-Type': "application/json" },
