...
export function useFormComposable() {
   ...
   * REACTIVE VARIABLES
   */
  const cpl_autoSaveCounter = ref(0);
  const cpl_initialFormData = ref("");
    const cplIsFormLoaded = ref(false);
  /**
   * FUNCTIONS
   */
  ...
  //it only saves if the initial data is not the same as the current for data
    function cpl_FnAutoSaveFormData(obj) {

    cpl_autoSaveCounter.value = cpl_autoSaveCounter.value + 1;
    if (cpl_autoSaveCounter.value == 1) {
        if ((obj.saveAlways != undefined && obj.saveAlways == true && obj.to.path.includes("/form/")) || ((cpl_initialFormData.value != JSON.stringify(this.form)) && obj.to.path.includes("/form/") && obj.status != undefined && obj.status == "Open")) {
  ...
  function cpl_FnSetInitialFormData() {
    var self = this;
    setTimeout(function () {
      cpl_initialFormData.value = JSON.stringify(self.form);
    }, 2000);
  }
    function cplOnAfterFormLoaded() {
        this.cplIsFormLoaded = true;
    }
  ...
  return {
    ...
    cpl_FnAutoSaveFormData, cpl_FnSetInitialFormData, cplIsFormLoaded,cplOnAfterFormLoaded
  };
}
