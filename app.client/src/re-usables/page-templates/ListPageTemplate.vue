    ...
    <div class="!text-xs">
        ...
        <div class="!top-40">
            ...
            <div class="flex flex-col">
                <div class="grow overflow-x-auto pb-20 rounded-t-lg">
                    <table class="table-auto w-full relative ">
                        <thead class="bg-gray-100 text-gray-600 font-semibold sticky top-0 border">
                            <tr class="text-xs">
                        ...
                        <tbody class="border">
                            <WTr v-if="!isFetchingData && records != null && records.length > 0" v-for="index in records.length" :index="index-1" @onRowClick="FnOnRowClick($event)" :isSelected="FnIsSelectedRow(index-1)" :isDoubleClick="pageProps.isRowDoubleClick">
                    ...
                    <div v-if="!isFetchingData && records != null" class="flex justify-center pt-1">
                        <div v-if="!isLoadingMore && records.length >= newRecordsLimit" class="mt-2 flex justify-center">
                            <WButton class="flex items-center rounded-sm bg-blue-500 !text-xs" @click="FnLoadMoreRecords()"><ChevronDoubleDownIcon class="h-4 w-4" /> Load More</WButton>
                        </div>
...
<script> 
  ...
  import { useListPageComposable } from '@/re-usables/composables/ListPageComposable.js';
    export default {
        ...
        setup() {
            ...
            return { router, prefetchComponent };
        },
        data() {
            return {
                records: [],
        ...
        created() {
            this.filter = this.FnUrlQuery();
            this.FnFetchData(this.appConfig.baseApiRoute + this.pageProps.controller + '/index' + this.filter);
        },
        mounted() {
                    ...
                    .then(data => {
                        if (data && data.errors) {
                            ...
                            if (data.fieldsProps != undefined) {
                                ...
                                //this.filterProps.fieldsProps = data.fieldsProps;
                            }
                ...
                if (clickType == "single" || clickType == "single+control") {
                    ...
                    if (this.pageProps.formRoute != undefined && this.pageProps.formRoute != null) {
                        if (this.pageProps.formEditable != undefined && this.pageProps.formEditable == false) {
                            this.router.push(this.pageProps.formRoute + "/view?recId="+ this.records[rowRecordIndex][this.pageProps.keys.recKey]);
                        } else {
                            this.router.push(this.pageProps.formRoute + "/edit?recId=" + this.records[rowRecordIndex][this.pageProps.keys.recKey]);
                        }
                    }
                }
            },
            ...
            FnIsSelectedRow(rowIndex) {
                if(this.selectedRows.length != 0){
                    for(var i=0;i < this.selectedRows.length;i++){
                        if(rowIndex == this.selectedRows[i]){
                            return true;
                        }
                    }
                }
        ...
        watch: {
            ...
            fetchResponse: function () {
                this.$emit('OnFetchData', this.fetchResponse);
            }
        }
    }
</script>
