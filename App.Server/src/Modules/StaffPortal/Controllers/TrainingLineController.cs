    ...
    public class TrainingLineController : ControllerBase
    {
        ...
        public IActionResult Index(string parentId)
        {
            try
            {
                ...
                response.header = GV.WSclient.ODATAClient(HttpContext).QyTrainingHeaders
                    .Where(obj => obj.Application_No == parentId)
                    .Where(obj => obj.Employee_no == GeneralController.SessionUser(HttpContext).userNo)
                    .FirstOrDefault();
                var baseQuery = GV.WSclient.ODATAClient(HttpContext).QyTrainingLines.AsQueryable();
                baseQuery = baseQuery.Where(obj => obj.Application_No == parentId);
                ...
                else
                {
                    ...
                    QyTrainingLines ModelInstance = new QyTrainingLines();
        ...
        public IActionResult GetFormData(string myAction, string parentId, int recId)
        {
            try
            {
                ...
                if (myAction != "create")
                {
                    response.formData = GV.WSclient.ODATAClient(HttpContext).QyTrainingLines
        ...
        [HttpPost]
        public IActionResult Store([FromBody] TrainingLine obj)
        {
            try
            {
                ...
                //
                var result = GV.WSclient.CuStaffWebportal(HttpContext).FnTrainingLineAsync(JsonSerializer.Serialize(obj)).Result;
        ...
        [HttpPost]
        public IActionResult Delete([FromBody] DeleteTrainingLine obj)
        {
            try
            {
                ...
                obj.parentId = obj.Application_No;
                var result = GV.WSclient.CuStaffWebportal(HttpContext).FnTrainingLineAsync(JsonSerializer.Serialize(obj)).Result;
                var response = JsonNode.Parse(result.return_value);
                if (response != null && response["status"]?.ToString() == "success")
                {
