    ...
    public class TrainingHeaderController : ControllerBase
    {
        ...
        public IActionResult Index(string status)
        {
            try
            {
                ...
                var baseQuery = GV.WSclient.ODATAClient(HttpContext).QyTrainingHeaders
                ...
                else
                {
                    ...
                    QyTrainingHeaders ModelInstance = new QyTrainingHeaders();
        ...
        public IActionResult GetFormData(string myAction, string recId, bool isApproval = false)
        {
            ...
            {
                dynamic response = new ExpandoObject();
                //var dimsStr = GV.DimsController.DocCompanyDims(HttpContext, GeneralController.DocumentTypes.TrainingApplicationHeader.GetDisplayName());
                //response.dims = JsonNode.Parse(dimsStr);
                if (!myAction.Contains("create"))
                {
                    var baseQuery = GV.WSclient.ODATAClient(HttpContext).QyTrainingHeaders
                        .Where(obj => obj.Application_No == recId)
                       .AsQueryable();
                    if (!isApproval)
                    {
                    ...
                    {
                        var approvers = GV.ApprovalMgt.GetApprovers(HttpContext, formData.Application_No, ApprovalDocumentTypes.TrainingApplication.GetDisplayName());
        ...
        //
        [HttpPost]
        public IActionResult Store([FromBody] TrainingHeader obj)
        {
            try
            {
                ...
                //validate dimensions
                //var dimsSetValidation = GV.DimsController.ValidateDims(HttpContext, GeneralController.DocumentTypes.TrainingApplicationHeader.GetDisplayName(), obj.dimensionSet);
                ...
                //
                var result = GV.WSclient.CuStaffWebportal(HttpContext).FnTrainingHeaderAsync(JsonSerializer.Serialize(obj)).Result;
        ...
        [HttpPost]
        public IActionResult Delete([FromBody] DeleteTraining obj)
        {
            try
            {
                obj.myAction = "delete";
        ...
        enum fieldNames { Application_No, Application_Date };
        [HttpGet]
        public IActionResult FieldsProps()
        {
            ...
            foreach (var field in Enum.GetValues<fieldNames>())
            {
                ...
                switch (field)
                {
                    case fieldNames.Application_No:
                        fieldProps.Caption = "Application No.";
                        break;
                    case fieldNames.Application_Date:
