...
namespace App.Server.src.Modules.ESS.Controllers
{
    ...
    public class ImprestRequestHeaderController : ControllerBase
    {
        [HttpGet]
        ...
        public IActionResult GetFormData(string myAction, string recId,bool isApproval=false)
        {
            try
            {
                ...
                if (!myAction.Contains("create"))
                {
                    ...
                    {
                        var approvers = GV.ApprovalMgt.GetApprovers(HttpContext, formData.No, ApprovalDocumentTypes.ImprestRequest.GetDisplayName());
        ...
        //
        [HttpPost]
        public IActionResult Store([FromBody] ImprestRequestHeader obj)
        {
            try
            {
                ...
                //
                var result = GV.WSclient.CuStaffWebportal(HttpContext).FnImprestRequestHeaderAsync(JsonSerializer.Serialize(obj)).Result;
        ...
        [HttpPost]
        public IActionResult Delete([FromBody] DeleteImprestRequest obj)
        {
            try
            {
                obj.myAction = "delete";
