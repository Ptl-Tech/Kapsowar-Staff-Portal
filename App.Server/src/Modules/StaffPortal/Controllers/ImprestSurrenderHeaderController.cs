...
namespace App.Server.src.Modules.ESS.Controllers
{
    ...
    public class ImprestSurrenderHeaderController : ControllerBase
    {
        [HttpGet]
        public IActionResult Index(string status)
        {
            try
            {
                ...
                var baseQuery = GV.WSclient.ODATAClient(HttpContext).QyImprestSurrenderHeaders
                ...
                else
                {
                    ...
                    QyImprestSurrenderHeaders ModelInstance = new QyImprestSurrenderHeaders();
        ...
        public IActionResult GetFormData(string myAction, string recId,bool isApproval = false)
        {
            ...
            {
                dynamic response = new ExpandoObject();
                var dimsStr = GV.DimsController.DocCompanyDims(HttpContext, GeneralController.DocumentTypes.ImprestSurrenderHeader.GetDisplayName());
                ...
                if (!myAction.Contains("create"))
                {
                    ...
                    {
                        var approvers = GV.ApprovalMgt.GetApprovers(HttpContext, formData.No, ApprovalDocumentTypes.ImprestSurrender.GetDisplayName());
        ...
        //
        [HttpPost]
        public IActionResult Store([FromBody] ImprestSurrenderHeader obj)
        {
            try
            {
                ...
                obj.receivedFrom = GeneralController.SessionUser(HttpContext).fullName;
                //
                var result = GV.WSclient.CuStaffWebportal(HttpContext).FnImprestSurrenderHeaderAsync(JsonSerializer.Serialize(obj)).Result;
        ...
        [HttpPost]
        public IActionResult Delete([FromBody] DeleteImprestSurrender obj)
        {
            try
            {
                obj.myAction = "delete";
        ...
        enum fieldNames { No, Surrender_Date, Amount };
        [HttpGet]
            ...
            foreach (var field in Enum.GetValues<fieldNames>())
            {
                ...
                switch (field)
                {
                    case fieldNames.No:
                        ...
                        break;
                    case fieldNames.Surrender_Date:
