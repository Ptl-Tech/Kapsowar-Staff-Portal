...
namespace App.Server.Modules.HMIS.Controllers
{
    ...
    public class ImprestRequestLineController : ControllerBase
    {
        [HttpGet] 
        public IActionResult Index(string parentId)
        {
            try
            {
                ...
                response.header = GV.WSclient.ODATAClient(HttpContext).QyImprestHeaders
                    .Where(obj => obj.No == parentId)
                    .FirstOrDefault();
                var baseQuery = GV.WSclient.ODATAClient(HttpContext).QyImprestLines.AsQueryable();
                ...
                else
                {
                    ...
                    QyImprestLines ModelInstance = new QyImprestLines();
        ...
        public IActionResult GetFormData(string myAction, string parentId, int recId)
        {
            try
            {
                ...
                if (myAction != "create")
                {
                    response.formData = GV.WSclient.ODATAClient(HttpContext).QyImprestLines
        ...
        [HttpPost]
        public IActionResult Store([FromBody] ImprestRequestLine obj)
        {
            try
            {
                ...
                //
                var result = GV.WSclient.CuStaffWebportal(HttpContext).FnImprestRequestLineAsync(JsonSerializer.Serialize(obj)).Result;
        ...
        [HttpPost]
        public IActionResult Delete([FromBody] DeleteImprestLine obj)
        {
            try
            {
                obj.staffNo = GeneralController.SessionUser(HttpContext).userNo;
