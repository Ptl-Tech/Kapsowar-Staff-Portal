...
namespace App.Server.Modules.HMIS.Controllers
{
    ...
    public class StoreReqLineController : ControllerBase
    {
        [HttpGet] 
        public IActionResult Index(string parentId)
        {
            try
            {
                ...
                response.header = GV.WSclient.ODATAClient(HttpContext).QyStoreReqHeaders
                    .Where(obj => obj.No == parentId)
                    .FirstOrDefault();
                var baseQuery = GV.WSclient.ODATAClient(HttpContext).QyStoreReqLines.AsQueryable();
                baseQuery = baseQuery.Where(obj => obj.Requistion_No == parentId);
                ...
                else
                {
                    ...
                    QyStoreReqLines ModelInstance = new QyStoreReqLines();
        ...
        public IActionResult GetFormData(string myAction, string parentId, int recId)
        {
            try
            {
                ...
                if (myAction != "create")
                {
                    response.formData = GV.WSclient.ODATAClient(HttpContext).QyStoreReqLines
                    .Where(obj => obj.Line_No == recId)
                    .Where(obj => obj.Requistion_No == parentId)
                    .FirstOrDefault();
                }
        ...
        [HttpPost]
        public IActionResult Store([FromBody] StoreRequestLine obj)
        {
            try
            {
                ...
                //
                var result = GV.WSclient.CuStaffWebportal(HttpContext).FnStoreRequestLineAsync(JsonSerializer.Serialize(obj)).Result;
        ...
        [HttpPost]
        public IActionResult Delete([FromBody] DeleteStoreRequestLine obj)
        {
            try
            {
                obj.staffNo = GeneralController.SessionUser(HttpContext).userNo;
